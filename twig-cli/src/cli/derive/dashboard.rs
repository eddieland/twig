...
#[command(about = "Show a comprehensive dashboard of local branches, PRs, and issues")]
#[command(
...
impl DeriveCommand for DashboardCommand {
  fn execute(self) -> Result<()> {
    // Create a runtime for async operations
    let rt = Runtime::new()?;

    // Get repository path (current or specified)
    let repo_path = if let Some(path) = self.repo {
    ...
    let branch_iter = match repo.branches(if self.include_remote {
      None
    } else {
          ...
          match rt.block_on(github_client.list_pull_requests(&owner, &repo_name, Some("open"), None)) {
            Ok(prs) => {
              for pr in prs {
                ...
                if self.mine && pr.user.login != creds.username {
                  continue;
                }
    ...
    if let Ok(creds) = get_jira_credentials() {
      ...
      if let Ok(jira_client) = create_jira_client(&jira_host, &creds.username, &creds.password) {
        // Set up JQL filters
        let assignee = if self.mine { Some("me") } else { None };
    ...
    match self.format.as_str() {
      ...
      _ => {
        // Output as text
        display_text_dashboard(&dashboard_data, self.include_remote);
      }
    }
...
mod tests {
  ...
  #[test]
  fn verify_cli() {
    DashboardCommand::command().debug_assert();
  }
}
