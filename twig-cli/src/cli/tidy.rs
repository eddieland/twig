...
pub fn handle_clean_command(clean: CleanArgs) -> Result<()> {
  ...
  if clean.aggressive {
    ...
    if !reparenting_operations.is_empty() {
      print_info(&format!(
        "Found {} reparenting opportunities:",
    ...
    if repo_state.remove_dependency(&child, &old_parent) {
      ...
      match repo_state.add_dependency(child.clone(), new_parent.clone()) {
        Ok(()) => {
          ...
          // If the old parent now has no children and no unique commits, mark it for
          // deletion
            ...
            && !branches_to_delete.contains(&old_parent)
          {
            ...
            print_info(&format!(
              "Added {} to deletion list (no children, no changes)",
              old_parent
            ));
          }
        }
  ...
  if deleted_count > 0 && reparented_count > 0 {
    print_success(&format!(
      "Clean complete: deleted {} branches, reparented {} branches.",
      deleted_count, reparented_count
    ));
...
fn find_cleanable_dependency_chain(
  ...
  // Only return the chain if it contains more than one element (indicating a
  // dependency chain) or if it's a single branch with no children (leaf node)
...
fn find_reparenting_opportunities(
  ...
  // Iterate through all dependencies to find intermediate branches with no
  // changes
  for dependency in &repo_state.dependencies {
...
mod tests {
  ...
  fn test_find_cleanable_dependency_chain_empty() {
    ...
    // A nonexistent branch with no children should return the branch itself as a
    // leaf
  ...
  fn test_find_cleanable_dependency_chain_single_branch() {
    ...
    // Test a branch with no children (should be considered cleanable as single
    // branch)
