...
impl ProgressIndicator {
  ...
  pub fn error(&mut self, error_message: &str) {
    ...
    let duration = self.start_time.elapsed();
    twig_core::output::print_error(&format!(
      "{} (failed after {:.2}s)",
      error_message,
...
impl ColorOutput {
  ...
  fn print_colored(&self, message: &str, color: Color) -> Result<()> {
    if self.colors_enabled {
      execute!(io::stdout(), SetForegroundColor(color), Print(message), ResetColor)?;
    } else {
      print!("{}", message);
    }
    println!();
...
impl UserHints {
  ...
  pub fn command_help_hint(command: &str, subcommand: Option<&str>) -> Option<String> {
    match (command, subcommand) {
      ("branch", Some("depend")) => Some(
        "Creates parent-child relationships between branches. Use: twig branch depend <child> <parent>".to_string(),
      ),
      ("cascade", None) => Some(
        "Performs cascading rebase from current branch to children. Use --show-graph to preview changes.".to_string(),
      ),
      ("tree", None) => Some("Shows branch dependency tree. Use --max-depth to limit levels shown.".to_string()),
      ("switch", None) => Some("Intelligent branch switching with automatic dependency detection.".to_string()),
      _ => None,
    }
  }
  ...
  pub fn repository_state_hints(repo_path: &std::path::Path) -> Vec<String> {
      ...
      .output()
    {
      ...
      if branches.iter().any(|b| b.contains("feature/") || b.contains("feat/"))
        && branches
          .iter()
          .any(|b| *b == "main" || *b == "master" || *b == "develop")
      {
...
mod tests {
  ...
  fn test_branch_name_suggestion() {
    ...
    assert_eq!(
      UserHints::suggest_branch_name("featrue/login", &branches),
