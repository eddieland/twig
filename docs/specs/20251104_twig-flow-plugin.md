...
- Schema overrides:
  - Hidden config file `${XDG_CONFIG_HOME}/twig/flow_renderer.toml` controls column layout via `FlowRendererSchemaConfig` (see `twig-core/src/git/renderer_config.rs`).
  - Missing config falls back to the compiled-in default schema so the plugin can rely on deterministic output.
...
| P1       | Define internal column schema configuration.                               | Document data model + default columns for renderer with hidden config override.                      | Enables future customization without public CLI surface.                          | ✅ Completed – config lives in `twig-core/src/git/renderer_config.rs` and loads from `${XDG_CONFIG_HOME}/twig/flow_renderer.toml` |
