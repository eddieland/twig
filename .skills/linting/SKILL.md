...
## name: linting description: >- Lint and format twig Rust code. Use when running clippy, rustfmt, checking code style, fixing warnings, or formatting. Covers cargo clippy, cargo fmt, and Make targets for the twig workspace.
...
1. `cargo clippy --fix --allow-dirty --workspace` — auto-fix clippy lints
1. Ruff format + check on `examples/plugins/twig-backup` (Python plugin examples)
...
Runs `cargo clippy --workspace -- -D warnings`. All warnings are treated as errors. This is what CI enforces.
...
- **`redundant_closure`** — Pass function directly instead of wrapping in closure

## Workspace-level lint bans

The workspace `Cargo.toml` explicitly **denies** these lints — clippy will treat them as errors when you run
`make lint`:
...
The project uses Rust **nightly** with clippy and rustfmt components pinned in `rust-toolchain.toml`. No need to install
them separately — `rustup` handles it automatically.
