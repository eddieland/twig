...
use twig_cli::enhanced_errors::{ErrorHandler, TwigError, ErrorCategory};
use twig_cli::user_experience::{ProgressIndicator, ColorOutput, UserHints};
...
fn test_levenshtein_distance_calculations() {
    ...
    // Access the private function through the public interface by testing suggestions
    let branches = vec!["feature".to_string(), "main".to_string()];
    
    // Distance of 1 (substitution)
    assert_eq!(UserHints::suggest_branch_name("featur", &branches), Some("feature".to_string()));
    
    // Distance of 1 (insertion)
    assert_eq!(UserHints::suggest_branch_name("featuree", &branches), Some("feature".to_string()));
    
    // Distance of 2 should still suggest
    assert_eq!(UserHints::suggest_branch_name("featue", &branches), Some("feature".to_string()));
