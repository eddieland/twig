...
fn test_tidy_preserves_chain_with_changes_in_middle() -> Result<()> {
  ...
  // Verify that only C was cleaned up appropriately
  // A should remain (has dependent B with changes)
  // B should remain (has changes)
  // C should be deleted (no changes from B)
  assert!(branch_exists(&repo, "branch-a")); // Should remain (has dependent with changes)
