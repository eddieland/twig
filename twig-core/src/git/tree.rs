...
use crate::git::{
  BranchAnnotationValue, BranchEdge, BranchGraph, BranchGraphError, BranchName, BranchNode, BranchTableColorMode,
  BranchTableLinkMode, BranchTableLinks, BranchTableRenderer, BranchTableSchema, BranchTableStyle,
  ORPHAN_BRANCH_ANNOTATION_KEY, checkout_branch, extract_github_repo_from_url,
};
use crate::output::{format_command, print_error, print_warning};
use crate::state::RepoState;

/// Represents the branch selection side effects requested by the caller.
#[derive(Default)]
...
/// Load repository state from `.twig/state.json`, falling back to an empty
/// state with a warning.
pub fn load_repo_state(repo: &Repository) -> Result<RepoState> {
  let workdir = repo
    .workdir()
...
/// Switch to the configured root branch before rendering or reporting a warning
/// when missing.
pub fn select_root_branch(repo: &Repository, state: &RepoState) -> Result<BranchSelection> {
  if let Some(root_branch) = default_root_branch(state) {
    ...
    Ok(BranchSelection::default())
  }
}

/// Switch to the single configured parent of the current branch, if present.
pub fn select_parent_branch(repo: &Repository, state: &RepoState) -> Result<BranchSelection> {
...
/// Return the configured default root or fall back to the first known root
/// branch.
...
/// Resolve the current branch name unless the repository is in a detached HEAD
/// state.
...
/// Decide which branch should be used as the render root for the table output.
...
/// Attach orphaned branches to the configured default root so they render under
/// a single tree.
...
/// Mark orphaned branches with a metadata annotation.
...
/// Identify branches with no configured dependency parents or explicit roots.
...
/// Return a filtered graph containing matching branches and their ancestors.
...
/// Print a user-facing message for branch graph build failures.
pub fn handle_graph_error(err: BranchGraphError) {
  match err {
...
/// Render the branch table using the configured schema, style, and
/// highlighting.
pub fn render_branch_table(
  graph: &BranchGraph,
  root: &BranchName,
...
/// Show a hint about orphan annotations when they are present in the render
/// output.
pub fn display_orphan_note(orphaned: &BTreeSet<BranchName>) {
  if orphaned.is_empty() {
    return;
  }
...
/// Build link metadata for branch table rendering, honoring opt-out flags and
/// env overrides.
pub fn build_branch_table_links(repo: &Repository, disable_links: bool) -> BranchTableLinks {
...
/// Resolve a Jira base URL from the `JIRA_HOST` environment variable.
pub fn resolve_jira_base_url() -> Option<String> {
...
/// Determine the GitHub owner/repo tuple from the repository origin remote.
pub fn resolve_github_repo(repo: &Repository) -> Option<(String, String)> {
...
mod tests {
  ...
  use super::*;
  use crate::git::{BranchEdge, BranchGraph, BranchHead, BranchKind, BranchTopology};
