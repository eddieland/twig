//! # Branch switch helpers
//!
//! Shared logic for switching to branches based on Jira issues, GitHub pull
//! requests, or explicit branch names. This module powers both the core CLI and
//! external plugins.
...
pub fn detect_input_type(jira_parser: Option<&JiraTicketParser>, input: &str) -> InputType {
...
pub fn handle_github_pr_switch(
  gh: &GitHubClient,
  repo_path: &Path,
  pr_number: u32,
  create_if_missing: bool,
...
pub fn handle_root_switch(repo_path: &std::path::Path) -> Result<()> {
