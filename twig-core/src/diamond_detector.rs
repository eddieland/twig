...
impl<'a> DiamondDetector<'a> {
  ...
  fn construct_diamond(&self, ancestor: &str, merge_point: &str, merge_parents: &[String]) -> Option<DiamondPattern> {
    ...
    // For simple diamonds, the different merge_parents themselves represent
    // different paths
  ...
  fn is_diamond_contained(&self, inner: &DiamondPattern, outer: &DiamondPattern) -> bool {
    ...
    // from outer diamond's ancestor and inner's ancestor should be reachable from
    // outer's ancestor
...
mod tests {
  ...
  fn test_no_diamond_linear_chain() {
    ...
    branches.insert(
      "feature2".to_string(),
      create_test_branch("feature2", false, vec!["feature1".to_string()], vec![]),
    );
  ...
  fn test_nested_diamond_detection() {
    ...
    // Create nested diamonds: main -> feature1, feature2 -> inner_merge ->
    // outer_merge
    branches.insert(
      "main".to_string(),
      create_test_branch(
    ...
    branches.insert(
      ...
      create_test_branch(
        ...
        false,
        vec!["feature1".to_string(), "feature2".to_string()],
        vec!["outer_feature".to_string()],
      ),
    );
    branches.insert(
      "outer_feature".to_string(),
      create_test_branch(
        "outer_feature",
        false,
        vec!["inner_merge".to_string()],
    ...
    branches.insert(
      "outer_merge".to_string(),
      create_test_branch(
        "outer_merge",
        false,
        vec!["inner_merge".to_string(), "outer_feature".to_string()],
        vec![],
      ),
    );
