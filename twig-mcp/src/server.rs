...
impl McpServer {
  ...
  pub async fn run(&mut self) -> Result<()> {
    ...
    loop {
      line.clear();
      let bytes_read = reader.read_line(&mut line).await.context("Failed to read from stdin")?;
  ...
  async fn handle_message(&mut self, message: &str) -> Result<JsonRpcResponse> {
    let request: JsonRpcRequest = serde_json::from_str(message).context("Failed to parse JSON-RPC request")?;
    ...
    let result = match request.method.as_str() {
      ...
      method => {
        ...
        return Ok(JsonRpcResponse {
          ...
          id: request.id,
          result: None,
          error: Some(JsonRpcError::method_not_found(format!("Unknown method: {}", method))),
        });
      }
    };
  ...
  async fn handle_initialize(&mut self, params: Option<Value>) -> Result<Value> {
    let _params: InitializeParams =
      serde_json::from_value(params.ok_or_else(|| anyhow::anyhow!("Missing initialize params"))?)
  ...
  async fn handle_call_tool(&self, params: Option<Value>) -> Result<Value> {
    ...
    let call_params: CallToolParams =
      serde_json::from_value(params.ok_or_else(|| anyhow::anyhow!("Missing tool call params"))?)
  ...
  async fn handle_read_resource(&self, params: Option<Value>) -> Result<Value> {
    ...
    let read_params: ReadResourceParams =
      serde_json::from_value(params.ok_or_else(|| anyhow::anyhow!("Missing resource read params"))?)
