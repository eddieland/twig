...
pub fn create_worktree<P: AsRef<Path>>(repo_path: P, branch_name: &str) -> Result<PathBuf> {
    ...
    if !worktrees_dir.exists() {
        fs::create_dir_all(&worktrees_dir)
            .context(format!("Failed to create worktrees directory at {}", worktrees_dir.display()))?;
    }
    ...
    if branch_exists {
        ...
        repo.worktree(safe_branch_name.as_str(), worktree_path.as_path(), None)
        ...
        let head = repo.head()?;
        let target = head.target().ok_or_else(|| anyhow::anyhow!("HEAD is not a direct reference"))?;
...
pub fn list_worktrees<P: AsRef<Path>>(repo_path: P) -> Result<()> {
    use crate::utils::output::{format_command, format_repo_path, format_timestamp, print_header, print_info, print_warning};
    ...
    if worktree_names.len() == 0 {
        print_warning("No worktrees found for this repository.");
        print_info(&format!("Create one with {}", format_command("twig worktree create <branch-name>")));
        return Ok(());
    }
